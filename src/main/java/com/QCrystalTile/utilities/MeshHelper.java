...
public final class MeshHelper {
    ...
    public static boolean approximateEquality(Mesh m1, Mesh m2) {
        if (m1.getVertices().size() == m2.getVertices().size() && m1.getFaces().size() == m2.getFaces().size()) {
            ...
            /*if (Math.ceil(v1) == Math.ceil(v2)) {
                ...
                for (com.QCrystalTile.interfaces.Polygon p : m1.getFaces()) {
                ...
                for (com.QCrystalTile.interfaces.Polygon p : m2.getFaces()) {
                    double area = Math.floor(area(p));
                    if (!set2.contains(p)) {
                        set2.add(area);
                    }
    ...
    public static double volumeOfConvexHull(Mesh mesh) {
        ...
        if (!mesh.getFaces().isEmpty()) {
            ...
            for (com.QCrystalTile.interfaces.Polygon p : mesh.getFaces()) {
     ...
     * @return
     */
    public static double area(com.QCrystalTile.interfaces.Polygon poly) {
        double area = 0;
	 ...
	 * @return
	 */
	public static Polygon convertPolygonToJzyPolygon(com.QCrystalTile.interfaces.Polygon polygon) {
     ...
     * @return
     */
    public static PickablePolygon convertPolygonToPickablePolygon(com.QCrystalTile.interfaces.Polygon polygon) {
    ...
    public static Mesh convertQuickHullToMesh(QuickHull3D hull) {
        ...
        final List<com.QCrystalTile.interfaces.Polygon> faceList = new LinkedList<com.QCrystalTile.interfaces.Polygon>();
