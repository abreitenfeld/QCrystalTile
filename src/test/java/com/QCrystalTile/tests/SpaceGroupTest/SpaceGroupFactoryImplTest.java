...
import com.QCrystalTile.internationalshortsymbol.InternationalShortSymbolEnum;
import com.QCrystalTile.internationalshortsymbol.SpaceGroupFactoryImpl;
import com.QCrystalTile.interfaces.SpaceGroupEnumeration;
...
public class SpaceGroupFactoryImplTest {
	 ...
	 */ 
	@Test //(timeout=30000)
	public void testGeneratorSizes() throws InvalidSpaceGroupIDException, ParseException, IOException {
		...
		for (int groupIndex=0; groupIndex<230; groupIndex++ ) {
			System.out.print( "testing group " + (groupIndex+1) );
	...
	@Test //(timeout=30000)
	public void testResultSizes() throws InvalidSpaceGroupIDException, ParseException, IOException {
		Map<ID,Integer> checkNumbers = getMapIDToResultSize();

		SpaceGroupEnumeration<ID> sgEnum = new InternationalShortSymbolEnum();
		SpaceGroupFactoryImpl factory = new SpaceGroupFactoryImpl();
		for (int groupIndex=0; groupIndex<230; groupIndex++ ) {
			if( groupIndex+1 >=143 && groupIndex+1 <=194) {
				System.out.println("skipping Trigonal and hexagonal groups...");
				continue;
			}
			System.out.print( "testing group " + (groupIndex+1) );
			ID id = sgEnum.get(groupIndex);
			...
			//System.out.println( id.stringRepr() );
			SpaceGroup sp= factory.createSpaceGroup(id);
			List<Vector3D> moduloSpace = new ArrayList<Vector3D>();
			{
				moduloSpace.add( new Vector3D( new double[] { 1, 0, 0 } ) );
				moduloSpace.add( new Vector3D( new double[] { 0, 1, 0 } ) );
				moduloSpace.add( new Vector3D( new double[] { 0, 0, 1 } ) );
			}
			Set<Transformation> set = sp.getTransformations(
				moduloSpace,
				new Vector3D( new double[] {1, 1, 1})
			);

			int expectedResultSize = (Integer )checkNumbers.get(id);

			assertEquals(
				"group " + (groupIndex+1) + " (" + id.stringRepr() + "): checking number of transformations in the result set",
				expectedResultSize,
				set.size()
			);
		}
	...
	private Map<ID,Integer> getMapIDToGeneratorSize() {
		...
		try {
			...
			// 226
			ret.put( new ID("Fm-3c"), 8  );
			// 227
			ret.put( new ID("Fd-3m"), 8  );
			// 228
			ret.put( new ID("Fd-3c"), 8  );
			// 229
			ret.put( new ID("Im-3m"), 7   );
			// 230
			ret.put( new ID("Ia-3d"), 7   );
		}
	...
	private Map<ID,Integer> getMapIDToResultSize() {
		Map<ID,Integer> ret = new HashMap<ID,Integer>();
		try {
			// 1
			ret.put( new ID("P1"), 1 );
